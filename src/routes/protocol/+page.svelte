  ...
  <div use:reveal class="mb-14">
    <div class="relative">
      ...
      <div class="absolute inset-y-0 right-0 flex items-center pr-4">
        <span
          class="text-[10px] font-bold text-outline uppercase tracking-widest px-2 py-1 bg-surface-container-highest hidden sm:block"
        >
          {$t.protocol.shortcut}
        </span>
      </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
    ...
    <aside class="hidden lg:block lg:col-span-3">
      <div class="sticky top-24">
        <p
          class="text-[10px] font-black uppercase tracking-[0.4em] text-outline mb-6"
        >
        ...
        <nav class="flex flex-col gap-1">
          {#each $t.protocol.sections as section}
            ...
            >
              ...
              <span
                class="material-symbols-outlined text-sm opacity-0 group-hover:opacity-100 transition-opacity"
              >
                arrow_forward_ios
              </span>
            </a>
    ...
    <div class="lg:col-span-9 space-y-16">
      {#each filteredSections as section}
        <section id={section.id} use:reveal>
          <div class="flex items-baseline gap-4 mb-8">
            <h2
              class="text-2xl md:text-3xl font-black uppercase tracking-tight whitespace-nowrap"
            >
              {section.title}
            </h2>
          ...
          <div class="space-y-2">
            {#each section.faqs as faq, i}
              ...
              <div
                class="bg-surface-container border-b border-outline-variant/10"
              >
                <button
                ...
                >
                  <span
                    class="text-base font-bold tracking-tight uppercase
                ...
                {#if openItems.has(itemId)}
                  <div
                    class="px-6 pb-6 text-on-surface-variant leading-relaxed text-sm border-l-2 border-primary/30 ml-6"
                  >
                    {faq.a}
                  </div>
                {/if}
      ...
      {#if filteredSections.length === 0}
        <div class="text-center py-20">
          <span
            class="material-symbols-outlined text-4xl mb-4 block text-outline"
            >search_off</span
          >
          <p
            class="font-bold uppercase tracking-widest text-sm text-on-surface-variant"
          >
