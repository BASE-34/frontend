...
<div class="container mx-auto px-6 md:px-8 max-w-7xl pb-24">
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
    ...
    <div class="lg:col-span-7" use:reveal>
      {#if submitted}
        <div
          class="bg-surface-container border-l-4 border-primary p-12 flex flex-col items-center text-center"
        >
          <span
          ...
          <p class="text-on-surface-variant text-sm mb-8">
            {$lang === "ua"
              ? "Ми відповімо протягом 24-48 годин."
      ...
      {:else}
        ...
        >
          {#if errors.server}
            <div
              class="bg-error/10 border-l-4 border-error p-4 text-sm text-error font-bold"
            >
              {errors.server}
            </div>
          {/if}
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label
                class="text-[10px] font-bold uppercase tracking-[0.3em] text-on-surface-variant block mb-2"
                for="name"
              >
                {$t.contact.form_name}
              </label>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={$lang === "ua"
                ? "Опишіть ваш запит..."
                : "Describe your inquiry..."}>{values.message ?? ""}</textarea
            >
            {#if errors.message}
              <span class="text-[10px] text-error mt-1 block"
                >{errors.message}</span
    ...
    <div class="lg:col-span-5 flex flex-col gap-5" use:revealLeft>
      <div class="bg-surface-container-low p-8 border-l-4 border-primary">
        <h3
          class="font-black uppercase tracking-tight text-lg mb-8 text-primary"
        >
          {$t.contact.info_title}
        </h3>
        <div class="space-y-6">
          {#each [{ icon: "location_on", label: $t.contact.info_location, value: $t.contact.info_location_val }, { icon: "mail", label: $t.contact.info_email, value: $t.contact.info_email_val }, { icon: "send", label: $t.contact.info_telegram, value: $t.contact.info_telegram_val }] as info}
            <div class="flex gap-4">
              <span
                class="material-symbols-outlined text-primary flex-shrink-0"
                style="font-size:20px">{info.icon}</span
              >
              <div>
                <div
                  class="text-[10px] font-bold uppercase tracking-[0.3em] text-on-surface-variant mb-1"
                >
                  {info.label}
                </div>
      ...
      <div class="bg-surface-container p-6">
        <div
          class="text-[10px] font-black uppercase tracking-[0.3em] text-outline mb-5"
        >
          System Telemetry
        </div>
        <div class="space-y-2.5">
          {#each [["NODE", "KNU_PRIMARY"], ["COORDINATES", "50.4501°N / 30.5234°E"], ["STATUS", "ONLINE"], ["RESPONSE_TIME", "24-48h"], ["ENCRYPTION", "TLS_1.3"]] as [k, v], i}
            <div class="flex justify-between items-center">
              <span
                class="text-[9px] font-mono text-on-surface-variant/50 uppercase"
                >{k}</span
              >
              <span
                class="text-[9px] font-mono text-primary uppercase"
                use:decode={{ targetText: v, duration: 1000 }}>{v}</span
              >
            </div>
          {/each}
        </div>
      ...
      <!-- Map placeholder -->
      <div
        class="bg-surface-container-low p-6 border-l-4 border-outline-variant/30 flex items-center gap-4"
      >
        <span
